...
<html>
    ...
    if(isset($_POST["update"])){ //Onko painettu submit painiketta?
        ...
        if ($_POST['givenSukupuoli'] == null){
            $updatet['sex'] = $_SESSION['sex'];
        }else{
            $updatet['sex'] = $_POST['givenSukupuoli'];
        }

        //talletetaan kantaan
            try{
                $stm = $DBH->prepare("UPDATE TABLE kk_person SET (fname=:fname, lname=:lname, phone=:phone, sex=:sex);");
